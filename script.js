          ...
          ${S.products.map((p, idx) => {
            ...
            return `<tr class="${rowCls}">
              ...
              <td data-code="${p.code}" style="white-space:nowrap;" id="pmCodeCell_${p.id}">
                <div class="editable-cell" onclick="editPMCode(${JSON.stringify(p.id).replace(/"/g,'&quot;')})" title="Click to edit PM code">
              ...
              <td id="nameCell_${p.id}" data-name="${p.name.toLowerCase().replace(/"/g,'&quot;')}" style="font-weight:600;max-width:260px;">
                <div class="editable-cell" onclick="editName(${JSON.stringify(p.id).replace(/"/g,'&quot;')})" title="Click to edit name">
                  <span>${p.name}</span><span class="edit-hint">✏️</span>
                </div>
              </td>
              <td id="sizeCell_${p.id}" style="color:var(--text2);">
                <div class="editable-cell" onclick="editSize(${JSON.stringify(p.id).replace(/"/g,'&quot;')})" title="Click to edit size">
                  <span>${p.size||'—'}</span><span class="edit-hint">✏️</span>
                </div>
              </td>
              <td id="matCell_${p.id}" style="color:var(--text2);">
                <div class="editable-cell" onclick="editMat(${JSON.stringify(p.id).replace(/"/g,'&quot;')})" title="Click to edit material">
              ...
              <td>
                <div id="minStockCell_${p.id}">
                  ...
                  <button class="btn btn-outline btn-xs" style="margin-top:3px;display:block;" onclick="editMinStock(${JSON.stringify(p.id).replace(/"/g,'&quot;')})">✏️</button>
                </div>
              </td>
              ...
              <td>
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                  <button class="btn btn-outline btn-sm btn-xs" onclick="editStock(${JSON.stringify(p.id).replace(/"/g,'&quot;')})">✏️ Stock</button>
                  <button class="btn btn-danger btn-xs" onclick="deleteProd(${JSON.stringify(p.id).replace(/"/g,'&quot;')})">🗑️</button>
                </div>
              </td>
            </tr>`;
          }).join('')}
